...
const AnalyticsDashboard = () => {
  ...
  useEffect(() => {
    const date = dataTableTickets.filter(cases => formatDate(cases.createDate) === dateToday()).length
    const dateDayOne = dataTableTickets.filter(cases => formatDate(cases.createDate) === dateBeforeDay(1)).length
    const dateDayTwo = dataTableTickets.filter(cases => formatDate(cases.createDate) === dateBeforeDay(2)).length

    const dateMonth = dataTableTickets.filter(cases => formatDate(cases.createDate).substr(3) === dateToday('MM/YYYY')).length
    const dateMonthBefore = dataTableTickets.filter(cases => formatDate(cases.createDate).substr(3) === dateBeforeDay(1, 'months', 'MM/YYYY')).length
    const dateTwoMonthBefore = dataTableTickets.filter(cases => formatDate(cases.createDate).substr(3) === dateBeforeDay(2, 'months', 'MM/YYYY')).length
              ...
              <Col lg="6" md="6" xs="6">
                <TinyChartStats
                  ...
                  series={[
                    {
                      name: 'Casos',
                      data: [casesDayTwoBefore, casesDayOneBefore, casesTodayState],
                    },
                  ]}
                />
              </Col>
              <Col xs="6">
                <TinyChartStats
                  ...
                  series={[
                    {
                      name: 'Casos',
                      data: [casesTwoMonthBeforeState, casesMonthBeforeState, casesMonthState],
                    },
                  ]}
                />
              </Col>
